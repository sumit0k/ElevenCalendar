...
class WelcomeWindow(QMainWindow):
    def __init__(self, parent: QObject = None, flags: Qt.WindowFlags = Qt.WindowFlags()) -> None:
        ...
        self.setWindowTitle(_("Welcome to ElevenCalendar"))
...
class FirstRunSlide(BasicNavWidget):
    def __init__(self, parent=None) -> None:
        ...
        label1.setText(f"""
             <h1>{_("Welcome to Elevencalendar!")}</h1>
        ...
        label3.setText(f"""
             ...
             {_("Please make sure to install ElevenCalendar from official sources only. Also, using ElevenCalendar implies the acceptation of the <b>GPLv3 license</b>")}""")

        label2 = IconLabel(size=64)
        label2.setIcon("rocket_color.png")
        label2.setText(f"""
             <h3>{_("This wizard will help you configure and customize ElevenCalendar. Click Start to get started!")}</h3>
...
class LastSlide(BasicNavWidget):
    def __init__(self, parent=None) -> None:
        ...
        settings.setText(f"""
             <h3>{_("Customize ElevenCalendar even more")}</h3>
             {_("Open the settings window and customize ElevenCalendar even further.")}""")
        ...
        report.setText(f"""
             <h3>{_("View ElevenCalendar on GitHub")}</h3>
             {_("View ElevenCalendar's source code. From there, you can report bugs or suggest features, or even contribute direcly to The ElevenCalendar Project")}""")
        report.setButtonText(_("Open GitHub"))
        report.clicked.connect(lambda: os.startfile("https://github.com/marticliment/ElevenCalendar"))
...
class SelectFullScreenSlide(BasicNavWidget):
    def __init__(self, parent=None) -> None:
        ...
        label1.setText(f"""<h1>{_("Fullscreen behaviour")}</h1>
                       {_("ElevenCalendar can hide when there's a fullscreen window present (when you are watching a video, you are playing, etc.), but it can also show over those windows (It might be useful if you use fullscreened terminals, etc.).<br><br>Please select one of the following and click next to continue")}""")
