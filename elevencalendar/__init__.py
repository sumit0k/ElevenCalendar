try:
    try:
        ...
        print("---------------------------------------------------------------------------------------------------")
        print("")
        print(f"   ElevenCalendar's {versionName} (v{version}) log: Select all the text and hit Ctrl+C to copy it")
        ...
        def updateIfPossible(force = False):
            try:
                if(not(getSettings("DisableAutoCheckForUpdates")) or force):
                    ...
                    try:
                        response = urlopen("https://www.marticliment.com/versions/elevencalendar.ver")
                    except Exception as e:
                        report(e)
                        response = urlopen("https://versions.marticliment.com/versions/elevencalendar.ver")
                    ...
                    if float(new_version_number) > version:
                        ...
                        if((not(getSettings("DisableAutoInstallUpdates")) and not(getSettings("DisableAutoCheckForUpdates"))) or force):
                            if not getSettings("EnableSilentUpdates"):
                                showNotif.infoSignal.emit(_("ElevenCalendar Updater"), _("ElevenCalendar is downloading updates"))
                            try:
                            ...
                            url = "https://github.com/marticliment/ElevenCalendar/releases/latest/download/ElevenCalendar.Installer.exe"
                            filedata = urlopen(url)
                            datatowrite = filedata.read()
                            filename = ""
                            if not os.path.exists(TemporaryDirectory):
                                os.mkdir(TemporaryDirectory)
                            with open(os.path.join(TemporaryDirectory, "elevencalendar-updater.exe"), 'wb') as f:
                                f.write(datatowrite)
                                filename = f.name
                            if hashlib.sha256(datatowrite).hexdigest().lower() == provided_hash:
                                ...
                                showWarn.infoSignal.emit(("Updates found!"), f"ElevenCalendar Version {new_version_number} is available, but ElevenCalendar can't verify the authenticity of the package. Please go ElevenCalendar's homepage and download the latest version from there.\n\nDo you want to open the download page?")
                        else:
                            showNotif.infoSignal.emit(("Updates found!"), f"ElevenCalendar Version {new_version_number} is available. Go to ElevenCalendar's Settings to update")
        ...
        def isElevenCalendarRunningThread():
            nowTime = time.time()
            LockFile = f"ElevenCalendarRunning{nowTime}"
            LockFilePath = os.path.join(os.path.expanduser("~"), ".elevencalendar", LockFile)
            LockFileLocation = os.path.join(os.path.expanduser("~"), ".elevencalendar")
            try:
                ...
                try:
                    if os.path.isfile(os.path.join(LockFileLocation, "ReloadClocks")):
                        try:
                            ...
                            report(e)
                    for file in glob.glob(os.path.join(LockFileLocation, "ElevenCalendarRunning*")):
                        if(LockFilePath == file):
                            pass
                        else:
                            if(float(file.replace(os.path.join(LockFileLocation, "ElevenCalendarRunning"), "")) < nowTime): # If lockfile is older
                                try:
                                    os.remove(file)
                                    ...
                                    if os.path.exists(file):
                                        try:
                                            ...
                                            report(e)
                            elif float(file.replace(os.path.join(LockFileLocation, "ElevenCalendarRunning"), "")) > nowTime:
                                globals.newInstanceLaunched = True
        ...
        def wanrUserAboutUpdates(a, b):
            if(QMessageBox.question(SettingsWindow, a, b, QMessageBox.Open | QMessageBox.Cancel, QMessageBox.Open) == QMessageBox.Open):
                os.startfile("https://github.com/marticliment/ElevenCalendar/releases/latest")
        ...
        class Clock(QWidget):
            ...
            def __init__(self, dpix: float, dpiy: float, screen: QScreen, index: int, isCover: bool = False, isSecondary: bool = False):
                ...
                if f"_{screen.name()}_" in getSettingsValue("BlacklistedMonitors"):
                    ...
                    if self.TASKBAR_DOES_AUTOHIDE:
                        print("🟡 ElevenCalendar set to hide with the taskbar")
            ...
            def updateTextLoop(self) -> None:
                ...
                while shiboken.isValid(self):
                    try:
                        ...
                        timeStr = evaluate_expression_string(time.time(),curFormatClock.replace("\u200a", "hairsec"),self.internetTimeOffset, self.internetWeather).replace("hairsec", HAIRSEC_VAR)
            ...
            def loadInternetWeather(self):
                while shiboken.isValid(self):
                    if True or self.getSettings("EnableWeather"): # This settings value will be cached, so no CPU/HDD overload ;)
                        try:
                            dict = json.loads(urlopen(self.getSettingsValue("WeatherURL") if self.getSettingsValue("WeatherURL") else "https://api.open-meteo.com/v1/forecast?latitude=52.52&longitude=13.41&current=temperature_2m&forecast_days=1").read().decode("utf-8"))
                            if "current" in dict.keys(): # worldtimeapi.org
                                self.internetWeather = str(dict['current']['temperature_2m']) + " " + dict['current_units']['temperature_2m']
                                print("🔵 (openmeteo) Weather set to", self.internetWeather)
                            else:
                                print("🟠 (Failed) Weather set to", self.internetWeather)
                                showNotif.infoSignal.emit("Invalid Internet weather URL", "Supported weather APIs are from openmeteo")
                        except Exception as e:
                            report(e)
                        for i in range(getint(self.getSettingsValue("WeatherSyncInterval"), 3600)):
                            time.sleep(1)
                            if getSettings("ReloadWeather"):
                                setSettings("ReloadWeather", False, thread=True)
                                break
                    else:
        ...
        KillableThread(target=isElevenCalendarRunningThread, daemon=True, name="Main: Instance controller").start()
        ...
        if not(getSettings(f"Updated{versionName}Already")) and not(getSettings("EnableSilentUpdates")):
            ...
            if getSettings("DefaultPrefsLoaded"):
                showMessage(_("ElevenCalendar Updater"), _("ElevenCalendar has updated to version {0} successfully\nPlease see GitHub for the changelog").format(versionName), False)
        ...
        if not getSettings("DefaultPrefsLoaded"):
            ...
            showMessage("Welcome to ElevenCalendar", "You can customize ElevenCalendar from the ElevenCalendar Settings. You can search them on the start menu or right-clicking on any clock -> ElevenCalendar Settings", uBtn=False)
        ...
        import ctypes
        ctypes.windll.user32.MessageBoxW(None, "Your ElevenCalendar installation appears to have missing or corrupt components. Please reinstall ElevenCalendar.\n\n"+tracebacc, "ElevenCalendar Crash handler", 0x00000010)
except Exception as e:
