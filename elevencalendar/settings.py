...
class SettingsUI(QMainWindow):
    ...
    def __init__(self):
        ...
        layout.addSpacing(0)
        self.title = QLabel(_("ElevenCalendar Settings"))
        ...
        self.selectedLanguage = QSettingsComboBox(_("ElevenCalendar's language")+" (Language)", _("Change")) # The non-translated (Language) string is there to let people know what the language option is if you accidentaly change the language
        ...
        def restartElevenCalendarByLangChange():
            subprocess.run(str("start /B \"\" \""+sys.executable)+"\" --settings", shell=True)
            globals.app.quit()

        self.selectedLanguage.restartButton.clicked.connect(restartElevenCalendarByLangChange)
        ...
        self.enableSystemTray = QSettingsCheckBox(_("Show ElevenCalendar on system tray"))
        ...
        clickActions = {
            ...
            _("Toggle ElevenCalendar setting"): "toggle_setting_click",
            _("Disabled"): "f20",
        ...
        doubleActions = {
            ...
            _("Toggle ElevenCalendar setting"): "toggle_setting_dblclick",
            _("Disabled"): "f20",
            _("Open quick settings"): "Win+A",
            _("Show desktop"): "Win+d",
            _("Open run dialog"): "Win+R",
        ...
        middleActions = {
            ...
            _("Empty the recycle bin"): "trashcan",
            _("Empty the recycle bin (Without confirmation)"): "trashcan_noconfirm",
            _("Toggle ElevenCalendar setting"): "toggle_setting_mdlclick",
            _("Disabled"): "f20",
            _("Open quick settings"): "Win+A",
            _("Show desktop"): "Win+d",
            _("Open run dialog"): "Win+R",
    ...
    {_("Now you should see the clock everywhere")}""")
        ...
        self.languageSettingsTitle = QSettingsTitle(_("About the language pack:"), getPath(f"lang_{self.iconMode}.png"), _("Language pack author(s), help translating ElevenCalendar"))
        layout.addWidget(self.languageSettingsTitle)

        translatorsHTMLList = _("ElevenCalendar has not been machine translated. The following users have been in charge of the translations:")
        ...
        self.openTranslateButton = QSettingsButton(_("Translate ElevenCalendar to your language"), _("Get started"))
        self.openTranslateButton.clicked.connect(lambda: os.startfile("https://github.com/marticliment/ElevenCalendar/wiki/#translating-elevencalendar"))
        ...
        def thirdPartyLicenses():
            ...
            msg.setObjectName("QMessageBox")
            msg.setTitle(_("Third Party Open-Source Software in Elevencalendar {0} (And their licenses)").format(versionName))
            colors = getColors()
            msg.setText(f"""
                <p>{_("ElevenCalendar is an Open-Source application made with the help of other libraries made by the community:")}</p><br>
            ...
            msg.addButton(_("More Info"), QDialogButtonBox.ButtonRole.ResetRole, lambda: os.startfile("https://github.com/marticliment/ElevenCalendar/wiki#third-party-libraries"))
            def closeAndQt():
                msg.close()
                QMessageBox.aboutQt(self, "ElevenCalendar - "+_("About Qt"))
        ...
        def exportSettings():
            ...
            try:
                rawstr = ""
                for file in glob.glob(os.path.join(os.path.expanduser("~"), ".elevencalendar/*")):
                ...
                fileName = QFileDialog.getSaveFileName(self, _("Export settings to a local file"), os.path.expanduser("~"), "ElevenCalendar Settings File (*.esf);;All Files (*.*)")
                if fileName[0] != "":
        ...
        def importSettings():
            nonlocal self
            try:
                fileName = QFileDialog.getOpenFileName(self, _("Import settings from a local file"), os.path.expanduser("~"), "ElevenCalendar Settings File (*.esf);;All Files (*.*)")
                if fileName:
        ...
        self.aboutTitle = QSettingsTitle(_("About ElevenCalendar version {0}:").format(versionName), getPath(f"about_{self.iconMode}.png"), _("Info, report a bug, submit a feature request, donate, about"))
        layout.addWidget(self.aboutTitle)
        self.WebPageButton = QSettingsButton(_("View ElevenCalendar's homepage"), _("Open"))
        self.WebPageButton.clicked.connect(lambda: os.startfile("https://marticliment.com/elevencalendar/"))
        ...
        self.IssueButton.clicked.connect(lambda: os.startfile("https://github.com/marticliment/ElevenCalendar/issues/new/choose"))
        ...
        self.resetButton = QSettingsButton(_("Reset ElevenCalendar preferences to defaults"), _("Reset"))
        ...
        contributorsHTMLList = _("ElevenCalendar wouldn't have been possible with the help of our dear contributors:")
        ...
        self.helpButton.clicked.connect(lambda: os.startfile("https://github.com/marticliment/ElevenCalendar/wiki#Troubleshooting"))
        self.debbuggingTitle.addWidget(self.helpButton)
        self.logButton = QSettingsButton(_("Open ElevenCalendar's log"), _("Open"))
        ...
        hiddenButtonList = [
            _("ElevenCalendar version:") + " " + getAppVersion(),
            ...
            _("ElevenCalendar language locale:") + f" lang_{langName}",
        ]
    ...
    def openLogWindow(self):
        ...
        win.show()
        win.setWindowTitle(_("ElevenCalendar's log"))
        """
        pixmap = QPixmap(32, 32)
        pixmap.fill(Qt.transparent)
        """
...
class QSettingsComboBox(QWidget):
    ...
    def __init__(self, text="", unusedArgument="", parent=None, buttonEnabled: bool = True):
        ...
        self.setObjectName("stBtn")
        self.restartButton = QPushButton("Restart ElevenCalendar", self)
...
class QAnnouncements(QLabel):
    ...
    def loadAnnouncements(self, useHttps: bool = True):
        try:
            response = urlopen(f"http{'s' if useHttps else ''}://www.marticliment.com/resources/elevencalendar.announcement")
            ...
            try:
                ...
                self.file =  open(os.path.join(os.path.join(os.path.join(os.path.expanduser("~"), ".elevencalendar")), "announcement.png"), "wb")
...
class CustomSettings(SettingsUI):
    def __init__(self, id, data):
        ...
        self.setWindowTitle(_("Settings for clock {0} on the monitor {1} - ElevenCalendar Settings").format(data[0], data[1]))
