...
def evaluate_expression_string(d, s: str, offset=0, weather=""):
...
def getSettings(s: str, env: str = ""):
    ...
    try:
        if not settingsName in globals.settingsCache:
            v = os.path.exists(os.path.join(os.path.join(os.path.expanduser("~"), ".elevencalendar"), settingsName))
...
def setSettings(s: str, v: bool, r: bool = True, thread = False, env: str = ""):
    ...
    try:
        ...
        globals.settingsCache[settingsName] = v
        FILE = os.path.join(os.path.join(os.path.expanduser("~"), ".elevencalendar"), settingsName)
        if(v):
            open(FILE, "w").close()
        else:
            try:
...
def setSettingsValue(s: str, v: str, r: bool = True, env: str = ""):
    ...
    try:
        globals.settingsCache = {}
        globals.settingsCache[settingsName+"Value"] = v
        with open(os.path.join(os.path.join(os.path.expanduser("~"), ".elevencalendar"), settingsName), "w") as sf:
            sf.write(v)
...
class TaskbarIconTray(QSystemTrayIcon):
    def __init__(self, app=None):
        ...
        self.show()
        self.setToolTip("ElevenCalendar - "+_("Customize the clock on Windows 11"))
        menu = QMenu(_("ElevenCalendar"))

        menu.setWindowFlag(Qt.WindowStaysOnTopHint)
        menu.setWindowFlags(menu.windowFlags() | Qt.FramelessWindowHint)
        menu.setAttribute(Qt.WA_TranslucentBackground)
        menu.addSeparator()
        self.settingsAction = QAction(_("ElevenCalendar Settings"), app)
        ...
        menu.addSeparator()
        self.nameAction = QAction(_("ElevenCalendar v{0}").format(versionName), app)
        self.nameAction.setEnabled(False)
        menu.addAction(self.nameAction)
        menu.addSeparator()
        self.restartAction = QAction(_("Restart ElevenCalendar"), app)
        ...
        self.quitAction = QAction(_("Quit ElevenCalendar"), app)
    ...
    """)
            ...
            msg.setWindowTitle("ElevenCalendar was updated!")
            msg.show()
...
try:
    ...
    os.chdir(".elevencalendar")
except FileNotFoundError:
    os.mkdir(".elevencalendar")


def loadLangFile(file: str, bundled: bool = False) -> dict:
    try:
        path = os.path.join(os.path.expanduser("~"), ".elevencalendar/lang/"+file)
...
def updateLangFile(file: str):
    ...
    try:
        try:
            oldlang = open(os.path.join(os.path.expanduser("~"), ".elevencalendar/lang/"+file), "rb").read()
        except FileNotFoundError:
            oldlang = ""
        newlang = urlopen("https://raw.githubusercontent.com/marticliment/ElevenCalendar/main/elevencalendar/lang/"+file)
        if newlang.status == 200:
            langdata: bytes = newlang.read()
            if not os.path.isdir(os.path.join(os.path.expanduser("~"), ".elevencalendar/lang/")):
                os.makedirs(os.path.join(os.path.expanduser("~"), ".elevencalendar/lang/"))
            if oldlang != langdata:
                print("🟢 Updating outdated language file...")
                with open(os.path.join(os.path.expanduser("~"), ".elevencalendar/lang/"+file), "wb") as f:
                    f.write(langdata)
                    f.close()
...
if langName == "":
    ...
    ans = QInputDialog.getItem(w, "Welcome to ElevenCalendar", "Please select the language you want ElevenCalendar to use:", languageReference.values(), editable=False)
    w.deleteLater()
    w.close()
    if ans[1] == False:
